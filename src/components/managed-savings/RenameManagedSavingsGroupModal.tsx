...
import { renameManagedSavingsGroup } from "@/lib/actions/managed-savings-group-actions";

interface RenameManagedSavingsGroupModalProps {
...
export function RenameManagedSavingsGroupModal({
  group,
  isOpen,
  onClose,
  onSuccess,
}: RenameManagedSavingsGroupModalProps) {
  ...
  const handleRename = () => {
    ...
    startTransition(async () => {
      const result = await renameManagedSavingsGroup({ id: group.id, name });
      if (result.ok) {
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-border/20">
          <div className="flex items-center gap-3">
            <div className="rounded-lg bg-asset/15 p-2">
              <Pencil className="h-5 w-5 text-asset" />
            </div>
            <h2 className="text-lg font-bold text-foreground">
              {tGroups("renameGroupTitle")}
            </h2>
          </div>
          <button
            onClick={onClose}
