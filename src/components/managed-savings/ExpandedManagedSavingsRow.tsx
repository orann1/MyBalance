...
import { getTrackPerformance, type ManagedSavingsInvestment } from "@/lib/mock/managed-savings-data";

interface ExpandedManagedSavingsRowProps {
  investment: ManagedSavingsInvestment;
}

export function ExpandedManagedSavingsRow({ investment }: ExpandedManagedSavingsRowProps) {
  ...
  return (
    <div className="bg-gradient-to-b from-secondary/15 to-secondary/5 border-t border-border/30 p-5 expanded-row-in">
      <div className="space-y-4">
        {/* PUBLIC TRACK PERFORMANCE SECTION */}
        <div className="rounded-lg bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200/60 p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {[
              ...
              <div key={item.label} className="rounded-lg bg-white/80 border border-green-100 p-3 text-center">
                <p className="text-xs text-muted-foreground mb-1 font-medium">{item.label}</p>
                <p className="font-bold text-green-700">{formatPercent(item.value)}</p>
              </div>
            ))}
          </div>

          <div className="mt-3 flex items-start gap-2 p-2.5 rounded-lg bg-blue-50 border border-blue-100">
            <Info className="h-3.5 w-3.5 shrink-0 mt-0.5 text-blue-600 flex-shrink-0" />
