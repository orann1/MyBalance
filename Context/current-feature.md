...
Phase 2F-2 — Fund Scenario Comparison MVP (product-facing name; the underlying files/branch/internal terms still say "Fund Replacement Simulator" — see the QA fix round note below for why) — **COMPLETED AND VERIFIED (2026-07-16)**. Product Owner browser QA approved, including two focused fix rounds. Committed and merged into `master` from `feature/fund-replacement-simulator` (fast-forward merge, commit `4dbaa99de7742adb0fc367e8bfaca19632d8a2ad`); branch deleted after merge. See `Context/feature-history.md` for the full completed-phase record and Phase 2F-2 Implementation Notes below for implementation detail. No phase is currently active; Phase 2F-3 (interactive scenario editing) remains not started, with no scope defined or approved.
...
- **Phase 2F-1: Projection Engine Refactor** — **COMPLETED AND VERIFIED (2026-07-14)**. Product Owner browser QA approved. Committed and merged into `master` from `feature/projection-engine-refactor`, commit `50cd94d`. Behavior-preserving extraction of the Managed Savings projection formula into a reusable, scalar-input pure engine (`src/lib/financial/projection.ts`) in preparation for the future Fund Replacement Simulator. See Phase 2F-1 Implementation Notes below.
- **Phase 2F-2: Fund Scenario Comparison MVP** (product-facing name as of the QA fix rounds below; still internally referred to as "Fund Replacement Simulator" in file/component names and the branch name) — **COMPLETED AND VERIFIED (2026-07-16)**. Product Owner browser QA approved after two focused fix rounds (neutral wording/RTL/fee-source distinction, then a mobile document-overflow fix). Committed and merged into `master` from `feature/fund-replacement-simulator` (fast-forward merge, commit `4dbaa99de7742adb0fc367e8bfaca19632d8a2ad`); branch deleted after merge. Adds a compact scenario-comparison entry point to eligible candidate rows in Similar Tracks Comparison, opening a modal that compares a current-vs-candidate fund projection (same balance/contribution, different return/fee assumptions) using the Phase 2F-1 shared projection engine. See Phase 2F-2 Implementation Notes below and `Context/feature-history.md` for the full record.
- **Phase 2F-3 (interactive scenario editing)** — Not started; no scope defined or approved yet. Explicitly out of scope for Phase 2F-2 (balance/contribution/return/fee remain read-only except the custom horizon).
...
## Phase 2F-1 Implementation Notes (2026-07-14)

**COMPLETED AND VERIFIED** (2026-07-14). Product Owner browser QA approved. Committed and merged into `master` from `feature/projection-engine-refactor`, commit `50cd94d`. Behavior-preserving refactor only — no UI, i18n, schema, or product-behavior change.

- **New pure engine**: `src/lib/financial/projection.ts` (new file) exports two scalar, domain-agnostic functions: `projectCompoundingWithFee(input)` (fee-adjusted monthly-compounding projection, including the pre-existing zero-effective-rate guard) and `projectLinearWithoutFee(input)` (the approved 0%-return fallback: `currentBalance + monthlyContribution × months`, no fee, no growth). Both take/return plain scalar types (`ScalarProjectionInput`/`ScalarProjectionResult`) — no React, i18n, Prisma, `ManagedSavingsInvestment`, or any domain type. Intended to also back the future Fund Replacement Simulator (Phase 2F-2+) against two parameter sets without duplicating the formula. No comparison function, candidate-fund selection, or simulator UI was added — out of scope for this phase.
- **Domain-to-engine adapter unchanged in shape**: `src/lib/mock/managed-savings-data.ts`'s internal `runProjection`/`runZeroReturnProjection` helpers now map a `ManagedSavingsInvestment` + year list to/from the new engine's scalar input/output instead of computing the formula inline — the math itself is bit-for-bit identical, only extracted. `getDisplayAnnualReturn`, `getProjectionAnnualReturn`, `projectWithAvailableReturnOrZero`, `projectSimulations`, and `calculateTotalSummary` are unchanged in signature and behavior; they still decide *which* return rate/mode a holding uses (linked-fund return vs. 0% fallback) — the engine has no knowledge of `linkedPublicFund`, `trackPerformance`, product type, group, or peer data.
- **Live call sites unchanged**: no call site outside `managed-savings-data.ts` was modified — `ManagedSavingsGroupTable.tsx` (per-row table cells), `src/lib/managed-savings/summary.ts`'s `calculateProjectionTotals` (group + global totals row) and `calculateManagedSavingsSummary`, `ManagedSavingsGroupSummaryRow.tsx`, `ManagedSavingsPageClient.tsx` (top KPI cards), and the legacy `ManagedSavingsSummaryTable.tsx` all continue to call the same existing adapter functions with the same signatures.
- **Regression verification**: no Vitest is configured in this repo (`package.json` has no `test` script or `vitest` dependency) — per the task's own escape hatch, a temporary script (`scripts/tmp-verify-projection.ts`) was created, run via `npx tsx`, and deleted before completion. It compared a byte-for-byte inlined copy of the pre-refactor formulas against the new engine across 13 representative scenarios (zero balance/contribution, balance-only at 0%, contributions at 0%, positive return with/without fee, return-equals-fee zero-rate guard, 1-year and 30/40-year horizons, custom horizons 12 and 22, negative effective rate, decimal inputs, always-flat zero-contribution holding) at multiple year offsets each — 91 checks total, 0 failures. No permanent test file was added to the repo.
- **Numerical safety reviewed**: percentages are divided by 100 consistently for both return and fee (unchanged); the annuity term's division by `monthlyRate` is guarded when the annual return exactly offsets the fee (0% effective rate), falling back to the linear sum (unchanged pre-existing guard, now inside the shared engine); a negative effective rate (fee > return) is not specially guarded — it was not guarded before this refactor either, and produces a mathematically valid (if unusual) shrinking projection, unchanged; very long horizons (30/40+ years) and decimal money/fee inputs were verified against the reference formula with no divergence; zero-balance and zero-contribution combinations are covered.
- **No Prisma/schema change, no new i18n strings, no UI change.**

## Phase 2F-2 Implementation Notes (2026-07-14, finalized 2026-07-16)

**COMPLETED AND VERIFIED.** Branch `feature/fund-replacement-simulator`, started from a clean `master` at commit `3fafbcd`. Product Owner browser QA approved after two focused fix rounds (see below). Committed and merged into `master` (fast-forward, commit `4dbaa99de7742adb0fc367e8bfaca19632d8a2ad`); branch deleted after merge.

- **Goal**: an informational current-vs-candidate fund projection simulator, connecting existing capabilities (holding data, linked-fund return, peer candidate return/fee, the Phase 2F-1 shared projection engine) without duplicating the compounding formula.
- **Comparison layer**: new `src/lib/financial/fund-comparison.ts` — `compareFundProjections({ currentScenario, candidateScenario })` (pure, calls `projectCompoundingWithFee` for both scenarios, returns `{ years, currentProjectedValue, candidateProjectedValue, difference }`, difference = candidate − current) and `buildSimulatorHorizons(customYears)` (returns the sorted, de-duplicated `[1, 5, 10, customYears?]` horizon list). No React/i18n/Prisma knowledge, no persistence.
- **Entry point**: a compact `Calculator` icon action added per eligible candidate row in `SimilarTracksComparison.tsx`'s existing peer table (not a new row/column layout, not a clickable row). Eligibility:
  - **Current side** (whole-investment gate): requires `getDisplayAnnualReturn(investment) != null` (the linked fund's real `latestAnnualized5YrReturn` — never a mock/0% fallback). When absent, the action column is omitted entirely for that holding's comparison table — no row shows it.
  - **Candidate side** (per-row gate): `!row.isTarget && Number.isFinite(row.annualized5YrReturn) && Number.isFinite(row.avgAnnualManagementFee)`. The user's own row and the peer-average row never show the action.
- **Candidate data addition**: `PeerComparisonRow` (`src/lib/public-funds/peer-comparison.ts`) gained two fields, both added for the simulator only and **not** rendered as new `SimilarTracksComparison` table columns (that table's existing 7-column layout is unchanged): `managingCompany` (identity display) and `annualized5YrReturn` (projection input — the annualized, non-cumulative return, additionally selected from the same already-queried `FundReturn` period row). **Important correctness note**: the pre-existing `trailing5YrReturn` field (used by the `SimilarTracksComparison` table's "5-year return" column, unchanged) is a *cumulative* 5-year return, not an annual rate — plugging it directly into the compounding formula's `annualReturnPercent` was tried during implementation and produced an absurd result (e.g. a displayed "111.83% annual return" and a ~₪933M/10yr projection for one real local-DB candidate). The simulator therefore uses `annualized5YrReturn` instead, verified via browser QA to produce sane output (e.g. ~16% for the same candidate) for the same underlying `FundReturn` row.
- **Modal**: new `src/components/managed-savings/FundReplacementSimulatorModal.tsx`. Sections: current-vs-candidate fund identity (holding name, linked fund name/company/number vs. candidate fund name/company/number), a read-only assumptions table (starting balance and monthly contribution — identical for both scenarios; annual return assumption and annual management fee — different per scenario, with explanatory notes distinguishing "projection assumption" from "personal realized return" and "public average fee" from "personal fee"; projection horizon), an editable custom-horizon number input (1–50, default 15, the same range/default already used elsewhere in Managed Savings), a results table (1/5/10 years + custom horizon, de-duplicated, current/candidate/signed difference), a below-table custom-horizon summary line, and a compact informational disclaimer (no "recommended"/"switch"/"guaranteed" wording). Recalculates immediately on horizon change (no separate "Run" button, `useMemo` over the comparison calls). Reopening always resets the custom horizon to 15 (the modal fully unmounts on close — `SimilarTracksComparison` only renders it when a candidate is selected, state is not lifted).
- **i18n**: new `managedSavings.fundReplacementSimulator` namespace (title/subtitle/close, identity/section labels, assumptions labels + notes, horizon label, results labels, disclaimer) plus `managedSavings.similarTracks.table.actionColumnSr` and `managedSavings.similarTracks.action.tooltip`/`ariaLabel`, added to both `he.json`/`en.json`. All new UI text is translated; no hardcoded strings.
- **No schema/migration changes** — `annualized5YrReturn`/`managingCompany` were already-stored `PublicFund`/`FundReturn` columns, only newly selected/exposed by `peer-comparison.ts`. No DB writes, no persisted simulation state, no analytics, no external calls, no logging of the combined personal+public inputs.
- **Calculation verification**: no Vitest configured in this repo — a temporary script (`scripts/tmp-verify-fund-comparison.ts`, run via `npx tsx`, deleted before completion) covered all 17 required scenarios from the task's QA checklist (equal/higher/lower return, lower/higher fee, zero balance with contributions, positive balance with zero contribution, 1/5/10-year horizons matching direct engine calls, custom horizons 12/22, custom horizon matching a baseline value with no duplicate row, decimal inputs, signed negative/zero currency formatting) — 24 checks, 0 failures.
- **Browser QA**: headless-Chromium Playwright scripts against a freshly rebuilt dev server (`rm -rf .next` + restart — the same stale dev-data-cache gotcha documented in the Phase 2E-1 QA notes was hit again and required the same fix) covered `/managed-savings` (he/RTL desktop + mobile 390px) and `/en/managed-savings` (en/LTR desktop): comparison table still renders, 10 eligible candidate action buttons found (peer-average and user's-own row correctly excluded), modal opens with the correct candidate pre-selected, identity/assumptions/results data verified correct against the underlying DB values, custom horizon (15→22, 15→5) updates the results table immediately with no duplicate row when matching a baseline horizon, no horizontal overflow in the modal at any viewport, modal closes via the X button, zero browser console errors in all three runs after the i18n/data fixes below.
- **Two implementation-time issues found and fixed via this same QA loop** (not shipped, caught before completion): (1) a duplicate JSON `"table"` key was initially added inside the `similarTracks` message namespace instead of merging into the existing `table` object — this silently shadowed the pre-existing `table.rank`/`fundName`/etc. keys (last-key-wins in JSON), which briefly broke the *existing*, already-shipped Similar Tracks Comparison table's labels; fixed by merging the new `actionColumnSr` key into the existing `table` object instead of introducing a second one. (2) the `trailing5YrReturn`-vs-`annualized5YrReturn` correctness issue described above.

### Phase 2F-2 QA fix round (2026-07-16, same branch, later committed and merged into `master` as part of the final Phase 2F-2 commit)

Product Owner browser QA identified UX, wording, and RTL issues, fixed on the same branch (no new branch, no commit). Presentation/i18n/RTL only — no calculation, schema, or eligibility-rule changes.

- **Product-facing rename**: all visible modal/action text changed from a replacement/switch framing to a neutral scenario-comparison framing — modal title `השוואת תרחישי קרנות` / `Fund Scenario Comparison`, subtitle, results heading (`results.title`), candidate-row tooltip/`aria-label` (`similarTracks.action.*`), and the sr-only action-column header (`similarTracks.table.actionColumnSr`). Internal file/component names (`FundReplacementSimulatorModal.tsx`), the branch name, and the i18n namespace key `fundReplacementSimulator` were intentionally left unchanged (out of scope — only user-facing text was in scope) and are referenced by their old internal name elsewhere in this doc.
- **Cumulative-vs-annualized return explanation**: a new, visibly-sized (not a tiny footnote) info row — `assumptions.returnMethodologyNote` — now sits directly under the assumptions table, explaining that the comparison uses the annualized 5-year return while the Similar Tracks table shows the cumulative 5-year return. No calculation change; `annualized5YrReturn` remains the simulator's input (unchanged from the correctness fix in the initial implementation).
- **Fee-source distinction**: the annual-fee assumption row now shows a small caption directly under each numeric value — `assumptions.currentFeeSourceLabel` ("Your entered fee") under the current fund's fee, `assumptions.candidateFeeSourceLabel` ("Reported public average") under the candidate's fee — replacing the previous generic paragraph notes, which risked reading as equivalent values.
- **Readable information block + short disclaimer**: the previous single dense paragraph disclaimer was split into a compact three-item list (`info.point1`/`point2`/`point3`, historical-data/candidate-fee/excluded-factors) under an `info.heading`, followed by a separate short, explicit non-recommendation disclaimer (`disclaimer`: "not a recommendation to switch funds"). Both use normal `text-xs` sizing, not a sub-11px footnote.
- **Neutral result wording**: `results.summaryPrefix` reworded to "Projected difference under these assumptions after {years} years"; new `results.summaryPositive`/`summaryNegative`/`summaryZero` sentence keys render a full neutral sentence ("the compared fund scenario is higher/lower by {amount}" / "there is no difference between the scenarios") below the signed number — no "profit", "gain", "savings", "winner", or "better" wording anywhere.
- **Neutral bottom-summary styling**: the custom-horizon summary card's background was already a neutral `bg-secondary/20` (not a strong filled green/red card) — unchanged. Added a small `TrendingUp`/`TrendingDown`/`Minus` icon next to the signed number so the sign is understandable without relying on color alone; the mathematical sign/direction (candidate − current) is unchanged.
- **RTL/LTR alignment fix**: `IdentityRow` (fund identity cards) changed from a `justify-between` row (label at one edge, value at the opposite edge — the reported bug) to a stacked label-then-value layout, both aligned `text-start` so they read from the same logical side in both locales. The assumptions and results tables changed their value columns from `text-end` to `text-start` for the same reason. A new small `Num` helper wraps only pure numeric/currency/percent tokens in `dir="ltr"` (preserving digit grouping, `+`/`-`, `%`, `₪` ordering) while the surrounding cell/container stays `text-start` — deliberately **not** used around mixed number+translated-word phrases (e.g. "{years} years"), since forcing a whole mixed phrase into `dir="ltr"` would reverse its natural word order; those cells wrap only the bare number in `Num` and leave the trailing unit word as plain text.
- **Portal fix for a real mobile-layout bug found during this round's own browser QA** (not in the original fix list, but blocking the explicit "no horizontal overflow" / "modal fits viewport" acceptance criteria): `ExpandedManagedSavingsRow`'s `expanded-row-in` entrance animation (`src/app/globals.css`) uses `animation: ... both`, which leaves `transform: translateY(0)` permanently applied after the animation finishes. A non-`none` `transform` on an ancestor creates a new containing block for `position: fixed` descendants — since `FundReplacementSimulatorModal` is mounted inside that animated row (via `SimilarTracksComparison`), its `fixed inset-0` backdrop was being positioned relative to that ancestor's (much wider, scrolled) box instead of the viewport, breaking the modal's layout entirely at narrow viewports (verified via Playwright: backdrop bounding box was `{x:16, width:1221}` instead of the viewport's `{x:0, width:390}` at a 390px-wide viewport). Fixed by rendering the modal via `createPortal(..., document.body)` — the first portal usage in this codebase; no other existing modal is ever mounted inside an animated/transformed ancestor, so none of them needed this. Verified via Playwright DOM assertions after the fix: backdrop `{x:0, y:0, width:390, height:844}` (exact viewport match), modal `{x:16, width:358}` (correctly fits within the 390px viewport with 16px margins).
- **QA performed**: `npm run lint` (clean), `npx tsc --noEmit` (clean), `npm run build` (exit 0), `npm run db:validate` (schema unchanged/valid). Calculation verification was **not** rerun — no calculation source file (`fund-comparison.ts`, `projection.ts`) was touched this round, presentation/i18n/RTL only. Browser QA via headless-Chromium Playwright scripts against a freshly rebuilt dev server (`rm -rf .next` + restart) on `/managed-savings` (he/RTL) and `/en/managed-savings` (en/LTR): 21 automated checks across desktop (1440px) he/RTL, desktop en/LTR, and mobile (390px) he/RTL — all passed (modal opens, correct new title text, no old "Fund Replacement Simulator"/"switch funds" wording present, no horizontal overflow, custom horizon 15→22 updates immediately with the new row, zero console errors). Desktop screenshots (he/RTL and en/LTR) were visually reviewed and confirmed correct alignment/wording/fee captions/methodology note/info block/disclaimer. **Mobile screenshots could not be captured** — Playwright's `page.screenshot()` returned a blank white image at narrow viewports (390px and 375px) in this sandbox even for the plain page with no modal open at all, confirming it is a screenshot-tooling limitation of this environment, not a rendering bug; mobile correctness was instead verified via DOM assertions (bounding boxes, computed styles, `innerText`, overflow checks, zero console errors), all of which passed.

### Phase 2F-2 mobile document-overflow fix round (2026-07-16, same branch, later committed and merged into `master` as part of the final Phase 2F-2 commit)

Product Owner reported a severe mobile regression: on Hebrew/RTL, the initial viewport appeared mostly blank and required horizontal scrolling to find the mobile header/menu; on English/LTR the initial layout was correct but the whole document could still be scrolled horizontally. Root-caused via Playwright DOM bisection (not guessed) to **two independent, unrelated causes**, both fixed:

1. **`MobileDrawer`'s off-canvas positioning** (`src/components/layout/MobileDrawer.tsx`): the closed drawer used `position: fixed` with a `translateX(±100%)` off-canvas offset. A `position: fixed` element is anchored to the viewport/initial containing block directly, bypassing an ordinary ancestor's `overflow` — so even though it was fully invisible (translated off-screen), its box (e.g. `{x:390, width:300}` on a 390px viewport, extending to x=690) was a genuine unclipped layout box. **Fix**: wrapped the `<aside>` in a new `fixed inset-0 overflow-hidden` container exactly matching the viewport, and changed the `<aside>` itself from `position: fixed` to `position: absolute` (relative to that new wrapper) — identical visual slide animation, but now genuinely clipped rather than merely invisible.
2. **`ManagedSavingsGroupTable`'s wide holdings table** (`src/components/managed-savings/ManagedSavingsGroupTable.tsx`): each group's table is intentionally wider than its `overflow-x-auto` wrapper (by design, for local horizontal scrolling). Bisection proved this wrapper's `overflow-x-auto` alone was **not** sufficient to prevent the table's full intrinsic width from inflating `document.documentElement.scrollWidth` — traced to `@dnd-kit/core`'s `DndContext`, which always renders a visually-hidden, `position: fixed` accessibility live-region (`DndLiveRegion-*`) inside this subtree; ordinary `overflow: auto/hidden/clip` does not establish a containing block for `position: fixed` descendants, and this descendant's mere presence was empirically found (via targeted CSS experiments) to prevent the wrapper from properly isolating the table's layout overflow from the document root, even though the table remained correctly scrollable/interactive locally. **Fix**: added `contain: paint` (Tailwind `contain-paint`) alongside the existing `overflow-x-auto` class on the wrapper — `contain: paint` both clips overflow and establishes the wrapper as the containing block for fixed/absolute descendants, which resolved it completely.
- **Root-level defense also added**: `overflow-x: clip` on `html`/`body` in `src/app/globals.css`. Empirically this alone did **not** reduce `document.documentElement.scrollWidth` for either bug above (kept as a documented, low-risk secondary safety net per the fix task's own guidance — it is not the primary fix for either issue and does not affect nested `overflow-x-auto` table scrolling).
- **Verification**: Playwright bisection (removing/altering suspect subtrees one at a time, confirmed via `document.documentElement.scrollWidth`) definitively isolated both causes before either fix was written — no guessing. Before fix: `docScrollWidth` was 1276px (he) / 1367px (en) on a 390px-wide viewport with two rendered holding groups. After fix: exactly 390px (he and en, modal closed, modal open, and drawer open) — verified across mobile (390px), a second mobile width (430px), and desktop (1440px, no regression). The intentionally-wide table wrapper still reports `scrollWidth (1305px) > clientWidth (356px)` post-fix, confirming local horizontal scrolling remains fully functional and was not globally suppressed.
- **QA performed**: `npm run lint` (clean), `npx tsc --noEmit` (clean), `npm run build` (exit 0), `npm run db:validate` (valid). No calculation file touched — verification not rerun. 30 automated Playwright checks covering all 8 required scenarios (he/en × mobile-modal-closed, mobile-modal-open, mobile-drawer-open, desktop) — all passed, zero console errors throughout. 5 required screenshots successfully captured this round (he mobile initial, he mobile drawer, he mobile simulator, en mobile initial, en mobile simulator) — unlike the prior QA fix round, mobile screenshot capture worked correctly this time, confirming the earlier blank-screenshot issue was transient to that session rather than a persistent environment limitation.
- **No regressions**: all Phase 2F-2 QA fix round items (neutral naming, cumulative-vs-annualized explanation, fee-source distinction, information block, neutral result wording, RTL/LTR alignment, portal-based modal containment) verified intact and unchanged in the same pass.

## Known Limitations / Deferred Items

- **Fund Scenario Comparison (Phase 2F-2) internal naming not renamed:** the product-facing name is "Fund Scenario Comparison" (Hebrew: "השוואת תרחישי קרנות"), but the component file (`FundReplacementSimulatorModal.tsx`), the i18n namespace key (`fundReplacementSimulator`), and the branch/commit history still reference "Fund Replacement Simulator" / "fund-replacement-simulator" — only user-facing text was renamed during the QA fix round; a broad internal rename was explicitly out of scope and remains a deferred cleanup item if ever desired.
