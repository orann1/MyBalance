...
Phase 2F-1 — Projection Engine Refactor (COMPLETED AND VERIFIED — Product Owner browser QA approved, committed and merged into `master` from `feature/projection-engine-refactor`; branch deleted after merge). No next phase has been started or approved yet. Phase 2F-2 (Fund Replacement Simulator UI) remains not started, with no scope defined or implementation status.
...
- **Phase 2F-1: Projection Engine Refactor** — **COMPLETED AND VERIFIED (2026-07-14)**. Product Owner browser QA approved. Committed and merged into `master` from `feature/projection-engine-refactor`. Behavior-preserving extraction of the Managed Savings projection formula into a reusable, scalar-input pure engine (`src/lib/financial/projection.ts`) in preparation for the future Fund Replacement Simulator. See Phase 2F-1 Implementation Notes below.
...
**COMPLETED AND VERIFIED** (2026-07-14). Product Owner browser QA approved. Committed and merged into `master` from `feature/projection-engine-refactor`. Behavior-preserving refactor only — no UI, i18n, schema, or product-behavior change.
