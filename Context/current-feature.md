...
**COMPLETED AND VERIFIED.** Branch `feature/fund-replacement-simulator`, started from a clean `master` at commit `3fafbcd`. Product Owner browser QA approved after two focused fix rounds (see below). Committed and merged into `master`; branch deleted after merge.
