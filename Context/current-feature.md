...
Phase 2F-1 — Projection Engine Refactor (COMPLETED AND VERIFIED — Product Owner browser QA approved, committed and merged into `master`, commit `50cd94d`; branch `feature/projection-engine-refactor` deleted after merge). No next phase has been started or approved yet. Phase 2F-2 (Fund Replacement Simulator UI) remains not started, with no scope defined or implementation status.
...
- **Phase 2F-2 (Fund Replacement Simulator UI) / Phase 2F-3 (interactive scenarios)** — Not started; no scope defined or approved yet. Explicitly out of scope for Phase 2F-1 (no comparison function, no candidate-fund selection, no simulator modal/UI, no new i18n strings).
